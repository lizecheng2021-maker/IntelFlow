<p align="center">
  ...
  <p align="center"><strong>AI 情报 Agent + 自媒体矩阵引擎 — 把每天的噪音变成双语日报，自动发布到你的全平台矩阵。</strong></p>
</p>
...
> 你需要的不是更多信息——是能帮你过滤、分析、输出判断的系统，还能帮你把内容发出去。
>
> IntelFlow 每天早上自动运行：AI Agent 围绕**你定义的主题**搜索全网，用**你的语气**写出中英双语深度日报，同步发布到微信公众号、飞书、WordPress、Twitter——你喝完咖啡，四个平台都发完了。
>
> 一个 API Key。自然语言配置。不需要维护爬虫。不需要订阅费。不需要编辑团队。

![IntelFlow Demo](assets/demo/demo.gif)

### 两个产品合一

**🧠 AI 情报系统** — 每日深度日报，由你的分析维度驱动，用你的语气输出，每期读起来都像你自己写的。

**📡 自媒体矩阵引擎** — 中英双语文章同步发布至你的全平台矩阵：博客、微信公众号、飞书知识库、Twitter 长推——一条流水线搞定所有平台。

全球唯一同时满足：

- **AI Agent 自主搜索** — 大模型直接搜索全网，不维护爬虫，不固定 RSS 列表
- **中英双语原生生成** — 不是翻译，两套独立编辑人格同步输出
- **60 秒自然语言配置** — 输入"AI 工具、加密、独立开发"→ AI Agent 帮你找信息源，完成配置
- **四端一键发布** — WordPress + 微信公众号 + 飞书 + Twitter，没有任何其他开源项目做到这四个
- **支持 8 家大模型供应商** — Claude、GPT-5、Gemini、Qwen3、Kimi、文心、智谱 GLM，或本地 Ollama
...
| 自媒体矩阵多平台发布 | 无 | 无 | 无发布流水线 | **有 — 微信+飞书+WordPress+Twitter** |
| AI Agent 自主搜索 | 无 — 编辑手动 | 无 — 仅 RSS | 无 — 固定源 | **有 — 大模型自主搜索全网** |
...
Morning Brew 这类商业产品需要大型编辑团队才能维持稳定的编辑风格。现有开源替代品是业余项目，没有多平台发布流水线。IntelFlow 是唯一一个同时做到生产级稳定性、真实编辑人格、全平台自媒体矩阵发布的自部署系统——配置文件驱动，不需要团队。
